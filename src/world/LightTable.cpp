...
int LightTable::getLight(int x, int y, int z) const {
	return vec[x][y][z];
}
int LightTable::getXSize() const {
	return xSize;
}
int LightTable::getYSize() const {
	return ySize;
}
...
void LightTable::verifyBrightness(int brightness) {
	if (!testBrightness(brightness)) {
		throw std::logic_error("light level is must be included in 0~15");
	}
}
}