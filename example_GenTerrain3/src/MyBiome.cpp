...
void MyBiome::onGenerateStructures(ofxPlanet::BlockTable& blockTable) {
        ...
        setWeightRange("Tree", glm::ivec3(0, 0, 0),
                       glm::ivec3(worldSize.x - 1, y + 1, worldSize.z - 1), 10);
...
void MyBiome::onGenerateCave(ofxPlanet::BlockTable& blockTable, int x, int y,
                             int z, float noise) {}

void MyBiome::onEndGenerateTerrain(ofxPlanet::BlockTable& blockTable) {
        int y = static_cast<int>(this->worldSize.y * 0.3f);
        int id =
            ofxPlanet::BlockPack::getCurrent()->getBlockIndexForName("Water");
        for (int x = 0; x < worldSize.x; x++) {
                for (int z = 0; z < worldSize.z; z++) {
                        for (int i = 0; i < y; i++) {
                                blockTable.setBlock(
                                    x, i, z, ofxPlanet::BlockPrefab(id, false));
                        }
                }
        }
}
